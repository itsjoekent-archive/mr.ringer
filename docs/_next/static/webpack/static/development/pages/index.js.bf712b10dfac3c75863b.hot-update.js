...
})(["display:flex;flex-direction:column;flex-wrap:wrap;width:100%;margin-top:48px;margin-bottom:48px;padding-top:16px;background-color:", ";border-top:2px solid ", ";border-bottom:2px solid ", ";position:relative;@media (min-width:1080px){height:90vh;max-height:600px;flex-direction:row;}&:before{content:'Mr. Ringer TI-84';", " font-size:", ";font-weight:700;font-style:italic;letter-spacing:", ";line-height:", ";text-transform:uppercase;white-space:nowrap;color:", ";background-color:", ";border:2px solid ", ";border-radius:4px;position:absolute;left:50%;transform:translate(-50%);top:-0.65em;padding-left:16px;padding-right:16px;}"], function (_ref) {
...
})(["display:flex;flex-direction:column;padding:16px;width:100%;max-height:200px;overflow-y:scroll;@media (min-width:1080px){width:20%;max-height:none;}"]);
...
})(["width:100%;padding:16px;position:relative;@media (min-width:1080px){width:40%;}"]);
  ...
  Object(react__WEBPACK_IMPORTED_MODULE_1__["useEffect"])(function () {
    function runScript() {
      ...
      function getPoint(x, y) {
        if (gridDimension === '2d') {
          var _gridPixelsPerUnit = gridPixelsPerUnit(),
              tallUnit = _gridPixelsPerUnit.tallUnit,
              wideUnit = _gridPixelsPerUnit.wideUnit;

          var canvasX = x * wideUnit + canvasWidth / 2;
          var canvasY = canvasHeight - y * tallUnit - canvasHeight / 2;
          return {
            x: canvasX,
            y: canvasY
          };
      ...
      function lineStyle() {
        ...
        var cap = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'round';
        var join = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'round';
      ...
      function grid(dimension, length) {
        ...
        if (dimension === '2d') {
          var frequency = Math.round(gridLength / 10);
          ctx.beginPath();
          lineStyle('#2f2f2f', 1, 'square');

          for (var xRow = -gridLength; xRow < gridLength; xRow += frequency) {
            for (var yRow = -gridLength; yRow < gridLength; yRow += frequency) {
              if (xRow === -gridLength || yRow === -gridLength || xRow === gridLength || yRow === gridLength) {
                continue;
              }

              var xGridLine = [getPoint(yRow, -gridLength), getPoint(yRow, gridLength)];
              var yGridLine = [getPoint(-gridLength, xRow), getPoint(gridLength, xRow)];
          ...
          ctx.closePath();
          var labelFrequency = Math.round(gridLength / 5);

          for (var _xRow = -gridLength; _xRow < gridLength; _xRow += labelFrequency) {
            for (var _yRow = -gridLength; _yRow < gridLength; _yRow += labelFrequency) {
              if (_xRow === -gridLength || _yRow === -gridLength || _xRow === gridLength || _yRow === gridLength || _xRow === 0 || _yRow === 0) {
                continue;
              }

              var xLabel = getPoint(_xRow, 0);
              var yLabel = getPoint(0, _yRow);
              ctx.font = '12px monospace';
              ctx.fillStyle = theme.colors.light;
              var xRowTextMetrics = ctx.measureText(_xRow);
              ctx.fillText(_xRow, xLabel.x - xRowTextMetrics.width / 2, xLabel.y + xRowTextMetrics.actualBoundingBoxAscent + 4);
              ctx.fillText(_yRow, yLabel.x + 4, yLabel.y + xRowTextMetrics.actualBoundingBoxAscent / 2);
            }
          }
      ...
      function arrow(fromX, fromY, toX, toY) {
        ...
        var _gridPixelsPerUnit2 = gridPixelsPerUnit(),
            wideUnit = _gridPixelsPerUnit2.wideUnit;

        var headLength = wideUnit * (gridLength * 0.0025);
        ctx.beginPath();
        ctx.moveTo(from.x, from.y);
        ctx.lineTo(to.x, to.y);
        var distance = [toX - fromX, toY - fromY];
        var angle = Math.atan2.apply(Math, distance);
        var headPoints = [getPoint(toX - headLength * Math.cos(angle - Math.PI / 6), toY - headLength * Math.sin(angle - Math.PI / 6)), getPoint(toX - headLength * Math.cos(angle + Math.PI / 6), toY - headLength * Math.sin(angle + Math.PI / 6))];
...
//# sourceMappingURL=index.js.bf712b10dfac3c75863b.hot-update.js.map